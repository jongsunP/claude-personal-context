...
### 다음 할 일
> ⚠️ **세션 재개 시 첫 번째 할 일: 수동 테스트**
> Claude가 로컬·스테이징 2연속을 통과했지만, 사용자가 직접 수동으로 최종 확인하지 못한 상태로 세션이 종료됨.
> 다음 세션 시작 시 코드 작업 전에 로컬 → 스테이징 각 2연속 수동 테스트를 먼저 진행할 것.

1. **수동 테스트 확인** — 로컬 `pnpm e2e:clinic` 2연속 → 스테이징 `pnpm e2e:clinic:stg` 2연속
2. **PR #4200 머지** — 수동 테스트 통과 후 진행
3. **다음 E2E 시나리오 추가** — `.claude/skills/e2e/references/03-scenario-backlog.md` 백로그 참고. `/e2e <시나리오>` 스킬로 진입
